---
aliases:
tags:
  - Civilization
  - Exploration
  - Vanilla
---
  ![Mongolia](https://assets.2k.com/1a6ngf98576c/4FdsE89lSRbgcDazfoJnSh/0b0f2bcf34992df7ebc2a315e9d27940/Civilizations_Desktop_Mongol.jpg)

[[Expansionist]], [[Militaristic]]

>*A name whispered in the nightmares of the settled lands, the Mongols stand poised to shatter the world—and remake it in a new image. With the thunder of bombards and roll of hooves, the hordes come. Lead them, and sow destruction among your enemies.*

## Unlocked
- Have three Siege units
- Improve three Horses
- Civilizations
	- [[Achaemenid Persia]]
	- [[Han]]
	- [[Silla Korea]]
- Leaders
	- [[Alexander the Great]]
	- [[Catherine the Great]]
	- [[Confucius]]
	- [[Genghis Khan]]
	- [[Xerxes, the Achaemenid]]
	- [[Xerxes, King of Kings]]

## Unique Ability
##### *Bokh*
- When you capture an opposing Settlement, gain the strongest Cavalry Unit you can currently train
- -50% Production towards training Settlers
- [Ant] Chariots are unlocked with Agriculture
- [Mod] +3 Settlement Limit in Advanced Starts

## Unique Infrastructure
##### Improvement: *Örtöö*
- +5 Gold
- Restores Movement to your Units at its location
- Cannot be placed on Rough, River, or a tile with a Feature

## Unique Units
##### Ranged Unit: *Keshig*
- Has +1 Movement and heals 15 HP after defeating an enemy Unit
- Counts as a Cavalry Unit
##### Army Commander: *Noyan*
- +50% Combat Strength from Flanking for Cavalry Units in the Command Radius
- Has additional Movement
- Can move after attacking

## Civics – Antiquity
##### *Origins*
- Tradition: **Baghatur I**
	- +3 Combat Strength for Cavalry Units
- +1 Settlement Limit
- +1 Tradition slot
##### *Foundation*
- Attribute Traditions: [[Expansionist|Fractal Cities]] and [[Militaristic|Warrior Class]] 
- +2 Settlement Limit
##### *Syncretism*
- Affirmation Tradition: **Jarugachi I**
	- +1 Influence and Production on Horse Resources, doubled in Captured Settlements from any Age

## Civics – Exploration
##### *Ulus*
- Improvement: **Örtöö**
- Wonder: **Erdene Zuu**
- Tradition: **Gerege**
	- +4 Happiness in Captured Settlements from any Age for each Resource assigned to them
- +1 Settlement Limit
##### *Four Hounds*
- Tradition: **Baghatur II**
	- +3 Combat Strength and +2 Movement for Cavalry Units
- +1 Settlement Limit
- +1 Tradition slot
##### *Yassa*
- Tradition: **Jarlig I**
	- +25% Production in Captured Cities from any Age
	- Captured Towns do not cost double to convert into Cities
- +2 Settlement Limit
```mermaid
graph LR;
	A[Ulus] --> B[Four Hounds];
	B --> C[Yassa];
```

## Civics – Modern
##### *Modernization*
- Tradition: **Jarlig II**
	- +50% Production in Captured Cities from any Age
	- Captured Towns do not cost double to convert into Cities
- +1 Settlement Limit
- +1 Tradition slot
##### *Administration*
- Attribute Traditions: [[Expansionist|Industrial Agriculture]] and [[Militaristic|Force Structuring]]
- +2 Settlement Limit
##### *Syncretism*
- Affirmation Tradition: **Jarugachi II**
	- +2 Influence and Production on Horse Resources, doubled in Captured Settlements from any Age
	- +50% Combat Strength from Flanking for Cavalry Units

## Associated Wonder
##### *Erdene Zuu*
- Unlocked for any Civilization by the *Authority II* Civic
- +2 Culture
- Creating a Cavalry Unit grants Culture equal to 25% of its cost
- Must be placed on Flat Terrain

## Starting Bias
- Horses

![Mongolia](https://assets.2k.com/1a6ngf98576c/4hKADNWsAjnj9TMynSJT1N/8e553a2975659aac06071a62fe81d9a2/Exploration-Era_Mongolia-City-02.png)

>*The steppes brace for a coming storm – one of horses, valor, and endless ambition. The Mongols ride forth.*
